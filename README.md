...
Artifacts appear under `src-tauri/target/release/bundle/` (e.g. `appimage/`).
...
- **`npm run version:bump -- patch|minor|major`** — bumps `package.json`, `src/version.ts`, and `src-tauri/tauri.conf.json`.
