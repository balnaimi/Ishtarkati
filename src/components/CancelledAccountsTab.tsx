...
import { DualCurrencyAmounts } from "./DualCurrencyAmounts";
import { SiteFavicon } from "./SiteFavicon";

export function CancelledAccountsTab() {
  ...
  const reload = useCallback(async () => {
    setLoading(true);
    try {
      const [list, prim] = await Promise.all([
        loadSubscriptionsCancelled(),
        getPrimaryCurrencyCode(),
      ]);
      setRows(list);
  ...
  if (loading) {
    return <p className="sk-text-hint">{t("common.loading")}</p>;
  }

  if (rows.length === 0) {
    return <p className="sk-text-hint">{t("cancelled.empty")}</p>;
  }
